...
class WaveletFeatureExtractor(nn.Module):
    def __init__(self, input_dim, input_channels, output_channels, n_layers, k, t, act=nn.LeakyReLU(), pool=nn.MaxPool1d(kernel_size=4, stride=4)):
        super().__init__()
        ...
        layers = [nn.Conv1d(input_channels, hidden_channels, kernel_size=3, padding=1), act, pool]
...
class ImageGFN(pl.LightningModule):
    def __init__(self, n_channels, img_dim, output_dim, num_gaussians, lr=1e-3, wavelet=False):
        ...
        if wavelet:
            ...
            self.feature_model = WaveletFeatureExtractor(self.y_len, n_channels + 2, n_channels, self.max_level - 1, self.k, self.step_size)
        else:
